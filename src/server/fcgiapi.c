...
static int fcgi_timeout = 6000;       //默认6s
...
int FCGX_VFPrintF(FCGX_Stream *stream, const char *format, va_list arg)
{
    ...
    while (f != fStop) {
        ...
        for (;;) {
			...
			if (!charPtrArg) charPtrArg = (char*)"(null)";
                        if(precision == -1) {
...
static int ProcessManagementRecord(int type, FCGX_Stream *stream)
{
    ...
    if(type == FCGI_GET_VALUES) {
        ...
        header = (FCGI_Header*)response;
        *(header) = MakeHeader(FCGI_GET_VALUES_RESULT, FCGI_NULL_REQUEST_ID,
                         len, paddedLen - len);
        FreeParams(&paramsPtr);
    } else {
        FCGI_UnknownTypeRecord* unkown_header = (FCGI_UnknownTypeRecord*)response;
        paddedLen = len = sizeof(FCGI_UnknownTypeBody);
        unkown_header->header
...
static void FillBuffProc(FCGX_Stream *stream)
{
    ...
    for (;;) {
        ...
        if(stream->rdNext == data->buffStop) {
            struct pollfd pf;
            pf.fd = data->reqDataPtr->ipcFd;
            pf.events = (POLLIN|POLLERR|POLLHUP);
            if (poll(&pf, 1, fcgi_timeout) <= 0) {
            //if (poll(&pf, 1, -1) <= 0) {
                SetError(stream, EAGAIN);
                return;
            }
            count = read(data->reqDataPtr->ipcFd, (char *)data->buff,
                            data->bufflen);
            if(count <= 0) {
...
int FCGX_Finish(void)
{
    // 返回值为1时，代表tcp连接已断开
    return FCGX_Finish_r(&the_request);
}
 ...
 *----------------------------------------------------------------------
 */
int FCGX_Finish_r(FCGX_Request *reqDataPtr)
{
    int close;
    ...
    /*
     * 有两种情况reqDataPtr->ipcFd的值为-1
     *     1.FCGX_Request对象刚刚初始化
     *     2.reqDataPtr->ipcFd连接已经断掉
     * 上述两中情况无需执行FCGX_Finish_r操作
     */
    
    if (reqDataPtr->ipcFd == -1) {
        return 0;
    }
    ...
    // 注意：FCGX_Request对象初始化时reqDataPtr->in为NULL，是不会close掉tcp连接的
    if (reqDataPtr->in) {
    ...
    return FCGX_Free(reqDataPtr, close);
}

/*
 * FCGX_Free返回值说明：
 *     返回值为-1时，说明传递的FCGX_Request对象的指针为NULL
 *     返回值为 0时，说明TCP连接不断开
 *     返回值为 1时，说明TCP连接已经断开
 * FCGX_Free返回值与FCGX_Finish_r函数返回值的含义是完全相同的
 */
int FCGX_Free(FCGX_Request * request, int close_flag)
{
    if (request == NULL) {
        return -1;
    }
...
int FCGX_Accept_r(FCGX_Request *reqDataPtr)
{
    ...
    reqDataPtr->ipcFd = accept(reqDataPtr->listen_sock, (struct sockaddr *)&addr, &len);
...
int FCGX_Process(FCGX_Request * r)
{
    r->isBeginProcessed = FALSE;
    r->in = NewReader(r, 8192, 0);
    FillBuffProc(r->in);
    if(!r->isBeginProcessed) {
        FCGX_Free(r, 1);
        return -1;
    }
    {
        ...
        switch(r->role) {
            case FCGI_RESPONDER:
                roleStr = (char*)"FCGI_ROLE=RESPONDER";
                break;
            case FCGI_AUTHORIZER:
                roleStr = (char*)"FCGI_ROLE=AUTHORIZER";
                break;
            case FCGI_FILTER:
                roleStr = (char*)"FCGI_ROLE=FILTER";
                break;
            default:
                FCGX_Free(r, 1);
                return -1;
        }
        r->paramsPtr = NewParams(30);
        PutParam(r->paramsPtr, StringCopy(roleStr));
    }
    SetReaderType(r->in, FCGI_PARAMS);
    if(ReadParams(r->paramsPtr, r->in) < 0) {
        FCGX_Free(r, 1);
        return -1;
    }
    /*
     * Finished reading the environment.  No errors occurred
     */
    SetReaderType(r->in, FCGI_STDIN);
    r->out = NewWriter(r, 8192, FCGI_STDOUT);
    r->err = NewWriter(r, 512, FCGI_STDERR);
    r->nWriters = 2;
    r->envp = r->paramsPtr->vec;
    return 0;
}
